...
# 确保脚本所在目录（new/）在 path 最前，以便 import train_sample_collectors
...
import transformers.models.wavlm.modeling_wavlm as _wavlm_mod

# 端到端时 input_values 带 grad_fn（非 leaf），WavLM 内部对 hidden_states.requires_grad = True 会报错，仅对 leaf 设置
def _wavlm_feat_forward_patched(self, input_values):
    hidden_states = input_values[:, None]
    if getattr(self, "_requires_grad", True) and self.training and hidden_states.is_leaf:
        hidden_states.requires_grad = True
    for conv_layer in self.conv_layers:
        hidden_states = conv_layer(hidden_states)
    return hidden_states
_wavlm_mod.WavLMFeatureEncoder.forward = _wavlm_feat_forward_patched
...
from train_sample_collectors import collect_train_samples
...
class LongWavLMDataset(Dataset):
    ...
    return_raw_waveform: 为 True 时（端到端 pathology）只返回原始波形 + 标签，不做掩码、不调 feature_extractor。
    """
    ...
    def __init__(
        ...
        self.feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(
            model_name_or_path
        ) if not return_raw_waveform else None

        if not return_raw_waveform and self.mask_strategy != "none" and self.masker is None:
...
class WavLMPathologyE2EWrapper(nn.Module):
    """
    端到端可学习病理权重的包装器：在 GPU 上对原始波形做软掩码后送入 WavLM。
    梯度从分类损失回传到 detector.weight_logits 与 masker.soft_threshold。
    """
    def __init__(
        self,
    ...
    def forward(self, waveforms: torch.Tensor) -> torch.Tensor:
        """
        waveforms: (B, T) 原始波形，已与 DataLoader 中 max_length 对齐。
        Returns: (B, num_labels) logits.
        注：detector 内部有 GPU->CPU(numpy) 特征计算，且逐条处理，大 batch 时仍会较慢。
        """
        B = waveforms.size(0)
        masked_list = []
        for i in range(B):
            w = waveforms[i : i + 1]  # (1, T) 保持二维避免 detector 里 mean(dim=0) 歧义
            masked_list.append(self.masker.generate_soft_masked_waveform(w).squeeze(0))
        masked_batch = torch.stack(masked_list, dim=0)
        # 可导归一化，与常见 WavLM 输入一致
        mean = masked_batch.mean(dim=1, keepdim=True)
        std = masked_batch.std(dim=1, keepdim=True).clamp(min=1e-5)
        input_values = (masked_batch - mean) / std
        return self.wavlm(input_values=input_values).logits


def train_one_epoch_e2e(
    model_wrapper: WavLMPathologyE2EWrapper,
    dataloader: DataLoader,
    optimizer: torch.optim.Optimizer,
    device: torch.device,
    criterion: nn.Module,
    ...
    *,
    ...
    """使用端到端包装器训练一轮；batch 需包含 'waveform' 与 'labels'。"""
    model_wrapper.train()
    total_loss = 0.0
    total_correct = 0
    total_count = 0
    ...
    for batch in dataloader:
        ...
        labels = batch["labels"].to(device, non_blocking=True)
        with torch.amp.autocast("cuda", enabled=use_amp):
            logits = model_wrapper(waveforms)
            loss = criterion(logits, labels) / gradient_accumulation_steps
        if scaler is not None and use_amp:
            scaler.scale(loss).backward()
        else:
    ...
    avg_loss = total_loss / total_count
    avg_acc = total_correct / total_count
    logger.info(f"Epoch {epoch} TRAIN (e2e) | loss={avg_loss:.4f}, acc={avg_acc:.4f}")
    return avg_loss, avg_acc
...
def evaluate_e2e(
    ...
    """端到端包装器验证；batch 需包含 'waveform' 与 'labels'。"""
    model_wrapper.eval()
    total_correct = 0
    total_count = 0
    with torch.no_grad():
        for batch in dataloader:
            ...
            total_count += labels.size(0)
    acc = total_correct / total_count if total_count > 0 else 0.0
    logger.info(f"VALIDATION (e2e) | acc={acc:.4f}")
    return acc
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="可选：权重初值文件，用于 init_weight_logits；不传则零初始化",
    )
    parser.add_argument(
        "--pathology_e2e",
        action="store_true",
        default=True,
        help="pathology 策略下使用端到端可学习权重（方案A），梯度回传到 detector 与 masker（默认开启）",
    )
    parser.add_argument(
        "--no_pathology_e2e",
        action="store_false",
        dest="pathology_e2e",
        help="关闭 pathology 端到端，仍用预搜权重在 Dataset 里做掩码",
    )
    ...
    # 1) 收集训练样本（见 train_sample_collectors）
    raw_samples, num_labels_override = collect_train_samples(
        str(data_root), binary=args.binary
    )
    ...
    use_e2e = (
        args.mask_strategy == "pathology"
        and getattr(args, "pathology_e2e", True)
    )
    if use_e2e:
        logger.info("Pathology 端到端模式（方案A）：权重与阈值在 GPU 上联合训练")
    ...
    if use_e2e:
        ...
        model = WavLMPathologyE2EWrapper(
            detector=detector,
            masker=masker,
            wavlm=wavlm_model,
        )
        ...
        model = wavlm_model.to(device)

    use_amp = args.use_amp and device.type == "cuda"
    if not use_e2e and getattr(torch, "compile", None) and args.compile:
    ...
    for epoch in range(1, args.num_epochs + 1):
        if use_e2e:
            train_loss, train_acc = train_one_epoch_e2e(
                model,
                train_loader,
                optimizer,
                device,
            ...
            val_acc = evaluate_e2e(model, val_loader, device, use_amp=use_amp)
        else:
        ...
        if val_acc > best_val_acc:
            ...
            save_dict = {
                ...
                "config": {
                    ...
                    "num_labels": num_labels,
                    "mask_strategy": args.mask_strategy,
                    "pathology_e2e": use_e2e,
                },
            }
            if use_e2e and hasattr(model, "detector") and hasattr(model.detector, "get_weights"):
                save_dict["pathology_weights"] = model.detector.get_weights()
