...
LOG_FILE="$LOG_DIR/pretrain_multi_corpus_random_${TIMESTAMP}.log"

# 记录开始时间
echo "==========================================" | tee -a "$LOG_FILE"
echo "Pretraining with RANDOM MASK" | tee -a "$LOG_FILE"
...
echo "    --output_dir ./checkpoints_pretrain_multi_corpus_random \\" | tee -a "$LOG_FILE"
...
echo "    --gpu_id 4" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

# 使用 tee 记录所有输出（包括 stdout 和 stderr）
python pretrain_with_disease_mask_update.py \
    --data_dirs "${DATA_DIRS[@]}" \
    --output_dir ./checkpoints_pretrain_multi_corpus_random \
    --batch_size 32 \
    --num_epochs 10 \
    --learning_rate 1e-5 \
    --pathology_threshold 75.0 \
    --device cuda \
    --gpu_id 4 2>&1 | tee -a "$LOG_FILE"
