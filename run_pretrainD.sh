...
# 创建日志目录
LOG_DIR="$SCRIPT_DIR/checkpoints_pretrain_disease/logs"
mkdir -p "$LOG_DIR"

# 生成日志文件名（包含时间戳）
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="$LOG_DIR/pretrain_disease_${TIMESTAMP}.log"
...
echo "python pretrain_with_disease_mask.py \\" | tee -a "$LOG_FILE"
echo "    --data_dirs ${DATA_DIRS[*]} \\" | tee -a "$LOG_FILE"
echo "    --output_dir ./checkpoints_pretrain_multi_corpus \\" | tee -a "$LOG_FILE"
echo "    --batch_size 2 \\" | tee -a "$LOG_FILE"
...
echo "    --gpu_id 3" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

# 使用 script 命令记录完整的交互式会话（包括输入和输出）
# 或者使用 tee 记录所有输出
python pretrain_with_disease_mask.py \
    --data_dirs "${DATA_DIRS[@]}" \
    --output_dir ./checkpoints_pretrain_multi_corpus \
    --batch_size 2 \
    --num_epochs 10 \
    --learning_rate 1e-5 \
    --use_disease_mask \
